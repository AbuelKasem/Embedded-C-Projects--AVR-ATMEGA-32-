...
#include"LCD_MA.h"				/* Include LCD header file */
...
int main(void)
{
	char array[10] = "teyt";				/* Declare array to be print */
	...
	_delay_ms(10);
	I2C_Start(EEPROM_Write_Addess);			/* Start I2C with device write address */
	I2C_Write(0x01);						/* Write start memory address */
	...
	for (int i = 0; i<strlen(array); i++)	/* Read data with acknowledgment(ack) */
		{
		LCD_SEND_CHAR_4bit(I2C_Read_Ack());
		}
